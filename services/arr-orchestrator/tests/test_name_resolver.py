...
class NameResolverTests(unittest.TestCase):
    ...
    def test_primary_and_all_source_fallbacks_share_one_total_budget(self):
        clock = [100.0]
        unrelated = movie_payload(348, "Alien", "Alien", 2001, "en")

        def search(params):
            query = str(params.get("query") or "").casefold()
            return {"results": [unrelated] if "momia" in query else []}
        ...
        session = AdvancingSession(
            {"/search/movie": search, "/movie/348": unrelated}
        )
        resolver = NameResolver(
            "token",
            "es-ES",
        ...
        rules = factory_identity_rules(
            resolver_http_timeout_ms=500,
            resolver_total_budget_ms=1000,
        )
        rules["resolver"]["search_limits"]["max_searches"] = 1
        input_root = self.input_file("release irreconocible xyz 2001 remux.mkv")
        infohash = "a" * 40
        now = time.time()
        job = {
            "name": "release irreconocible xyz 2001 remux.mkv",
            "category": "movies",
            "infohash": infohash,
        ...
        with patch(
            "arr_orchestrator.identity.resolver.service.time.monotonic",
            side_effect=lambda: clock[0],
        ):
            with self.assertRaises(ResolverUnavailable) as raised:
                resolver.resolve(job, input_root, rules)

        self.assertIn("Presupuesto de tiempo TMDb agotado", str(raised.exception))
        self.assertAlmostEqual(resolver._deadline, 101.0, places=6)
        self.assertEqual(len(session.calls), 3)
        self.assertAlmostEqual(session.calls[0][2], 0.5, places=6)
        self.assertAlmostEqual(session.calls[1][2], 0.5, places=6)
        self.assertAlmostEqual(session.calls[2][2], 0.2, places=6)
    ...
    def test_source_title_fallback_rescues_recoverable_movie_rejection(self):
        ...
        resolver, _ = self.resolver(
            {"/search/movie": {"results": [correct]}, "/movie/1734": correct}
        )

        payload = resolver.preview(
            "release irreconocible xyz 2001 remux",
            "movies",
            source_title="El regreso de la momia (4K-HDR) 2001 SPANISH 4K",
            source="buscador-pro",
        )
        ...
        self.assertEqual(
            payload["identity"]["source_context"]["source"], "buscador-pro"
        )
        self.assertTrue(payload["decision"]["source_fallback"]["applied"])
        self.assertIn("primary_attempt", payload)
        source_rows = [
            row
            for row in payload["candidates"][0]["breakdown"]
            if row["path"] == "resolver.source_title_fallback.score_bonus"
        ]
    ...
    def test_preview_reports_unsafe_source_title_after_primary_has_no_candidates(self):
        ...
        self.assertTrue(payload["ok"])
        self.assertEqual(payload["status"], "REJECTED_SOURCE_TITLE")
        self.assertEqual(payload["decision"]["status"], "REJECTED_SOURCE_TITLE")
        self.assertFalse(payload["decision"]["accepted"])
        self.assertFalse(payload["decision"]["source_title_policy_passed"])
        self.assertEqual(payload["details"]["reason_code"], "source_title_policy")
        self.assertEqual(payload["details"]["primary_status"], "NO_CANDIDATES")
        self.assertTrue(payload["primary_attempt"]["queries"])
        self.assertTrue(payload["queries"])
        self.assertEqual(payload["candidates"][0]["tmdb_id"], 348)
        self.assertEqual(
            payload["source_fallback"]["status"], "REJECTED_SOURCE_TITLE"
        )
        self.assertFalse(payload["source_fallback"]["applied"])
        self.assertEqual(
            payload["source_fallback"]["attempts"][0]["status"],
            "REJECTED_SOURCE_TITLE",
        )
        self.assertNotIn("identity", payload)

    def test_preview_reports_unsafe_source_title_when_fuzzy_year_conflicts(self):
        wrong_year = movie_payload(
            1734,
            "El regreso de la momia",
            "The Mummy Returns",
            1998,
        ...
        def search(params):
            query = str(params.get("query") or "").casefold()
            return {"results": [wrong_year] if "momia" in query else []}

        rules = factory_identity_rules()
        rules["resolver"]["source_title_fallback"]["min_similarity"] = 0.50
        resolver, _ = self.resolver(
            {"/search/movie": search, "/movie/1734": wrong_year}
        )
        ...
        payload = resolver.preview(
            ...
            "movies",
            rules,
            source_title="El regreso momia 2001",
            source="buscador-jackett",
        )
        ...
        self.assertEqual(payload["details"]["primary_status"], "NO_CANDIDATES")
        self.assertEqual(
            payload["details"]["source_fallback_attempts"][0]["reason_code"],
            "source_title_policy",
        )

    def test_source_title_fallback_disabled_preserves_primary_rejection(self):
        correct = movie_payload(1734, "El regreso de la momia", "The Mummy Returns", 2001)
        rules = factory_identity_rules()
        ...
        self.assertIn(payload["status"], {"REJECTED_SCORE", "REJECTED_MARGIN"})
        self.assertNotIn("source_fallback", payload["decision"])

    def test_source_title_never_rescues_an_unresolved_direct_imdb_identifier(self):
        correct = movie_payload(
            1734, "El regreso de la momia", "The Mummy Returns", 2001, "en"
        )
        resolver, session = self.resolver(
            {
                "/find/tt1234567": {"movie_results": []},
                "/search/movie": {"results": [correct]},
                "/movie/1734": correct,
            }
        )
        ...
        self.assertNotIn("source_fallback", payload["decision"])
        self.assertEqual(
            [call[0].split("/3", 1)[1] for call in session.calls],
            ["/find/tt1234567"],
        )

    def test_source_title_never_rescues_direct_or_forced_score_rejections(self):
        explicit = movie_payload(999, "Objetivo", "Target", 2024, "en")
        fallback = movie_payload(
            1734, "El regreso de la momia", "The Mummy Returns", 2001, "en"
        )
        base_rules = factory_identity_rules()
        base_rules["resolver"]["scoring"]["direct_identity"] = 0
        base_rules["resolver"]["acceptance"]["direct_ids_bypass"] = False
        base_rules["resolver"]["acceptance"]["forced_bypass"] = False

        cases = (
            ("Objetivo.tmdb-999.2024.mkv", base_rules, "tmdb_id"),
            (
                "Objetivo.2024.mkv",
                {
                    **base_rules,
                    "resolver": {
                        **base_rules["resolver"],
                        "forced_matches": {
                            **base_rules["resolver"]["forced_matches"],
                            "movies": ["Objetivo | 2024 | 999"],
                        },
                    },
                },
                "forced_match",
        ...
        for name, rules, expected_source in cases:
            with self.subTest(expected_source=expected_source):
                resolver, session = self.resolver(
                    {
                        "/movie/999": explicit,
                        "/search/movie": {"results": [fallback]},
                        "/movie/1734": fallback,
                    }
                )
                payload = resolver.preview(
                ...
                self.assertEqual(payload["status"], "REJECTED_SCORE")
                self.assertEqual(payload["decision"]["source"], expected_source)
                self.assertNotIn("source_fallback", payload["decision"])
                self.assertEqual(
                    [call[0].split("/3", 1)[1] for call in session.calls],
                    ["/movie/999"],
    ...
    def test_clear_physical_identity_does_not_score_source_title(self):
        correct = movie_payload(348, "Alien, el octavo pasajero", "Alien", 1979, "en")
        resolver, _ = self.resolver(
            {"/search/movie": {"results": [correct]}, "/movie/348": correct}
        )

        payload = resolver.preview(
            "Alien 1979 remux",
            "movies",
            source_title="Otra película completamente distinta 2024",
            source="buscador-jackett",
        )

        self.assertEqual(payload["status"], "ACCEPTED")
        self.assertEqual(payload["identity"]["tmdb_id"], 348)
        self.assertEqual(payload["identity"]["source"], "search")
        self.assertNotIn("source_fallback", payload["decision"])
        self.assertFalse(
            any(
                row["path"] == "resolver.source_title_fallback.score_bonus"
                for row in payload["candidates"][0]["breakdown"]
            )
        )

    def test_tv_source_fallback_keeps_physical_episode_coordinates(self):
        correct = tv_payload(999, "The Show", "The Show", 2020, seasons=5, original_language="en")
        resolver, _ = self.resolver(
            {"/search/tv": {"results": [correct]}, "/tv/999": correct}
        )

        payload = resolver.preview(
            "archivo irreconocible S01E03",
            "tv",
            source_title="The Show S02E09 2020",
            source="buscador-jackett",
        )

        self.assertEqual(payload["status"], "ACCEPTED")
        self.assertEqual(payload["identity"]["tmdb_id"], 999)
        self.assertEqual(payload["identity"]["season"], 1)
        self.assertEqual(payload["identity"]["episodes"], [3])

    def test_tv_source_fallback_merges_physical_season_with_source_episode(self):
        correct = tv_payload(
            999, "The Show", "The Show", 2020, seasons=5, original_language="en"
        )

        def search(params):
            return {
                "results": [correct]
                if "show" in str(params.get("query") or "").casefold()
                else []
            }

        resolver, _ = self.resolver({"/search/tv": search, "/tv/999": correct})

        payload = resolver.preview(
            "archivo irreconocible S01",
            "tv",
            source_title="The Show S02E09 2020",
            source="buscador-jackett",
        )
        ...
        self.assertEqual(payload["status"], "ACCEPTED")
        self.assertEqual(payload["identity"]["season"], 1)
        self.assertEqual(payload["identity"]["episodes"], [9])

    def test_tv_source_fallback_merges_physical_episode_with_source_season(self):
        correct = tv_payload(
            999, "The Show", "The Show", 2020, seasons=5, original_language="en"
        )
        ...
        payload = resolver.preview(
            "archivo irreconocible Episodio 3",
            "tv",
            source_title="The Show Season 2 2020",
            source="buscador-pro",
        )
    ...
    def test_source_fallback_never_turns_a_collection_into_one_movie(self):
        correct = movie_payload(
            1734,
            "El regreso de la momia",
            "The Mummy Returns",
        ...
        def search(params):
            return {
                "results": [correct]
                if "momia" in str(params.get("query") or "").casefold()
                else []
            }

        for physical_name in (
            "Mummy Collection 1999-2008",
            "Mummy Trilogy 1999",
            "Mummy Pack 1999",
        ):
            with self.subTest(physical_name=physical_name):
                resolver, session = self.resolver(
                    {"/search/movie": search, "/movie/1734": correct}
                )
                payload = resolver.preview(
                    physical_name,
                    "movies",
                ...
                self.assertEqual(payload["status"], "NO_CANDIDATES")
                self.assertEqual(
                    payload["details"]["source_fallback_block_reason"],
                    "ambiguous_collection",
                )
                self.assertFalse(
                    any(
                        "momia" in str(call[1].get("query") or "").casefold()
                        for call in session.calls
                    )
                )

    def test_source_fallback_cache_does_not_persist_the_source_title(self):
        raw_source_title = "Titulo Privado Unico 2001"
        correct = movie_payload(
            1734,
            "Titulo Privado Unico",
        ...
        def search(params):
            return {
                "results": [correct]
                if "privado" in str(params.get("query") or "").casefold()
                else []
            }
        ...
        input_root = self.input_file("release-irreconocible-cache.mkv")
        infohash = "d" * 40
        job = {
            "name": "release-irreconocible-cache.mkv",
            "category": "movies",
            "infohash": infohash,
            "source_meta_json": json.dumps(
                {
        ...
        identity = resolver.resolve(job, input_root)
        rows = self.database.connect().execute(
            "SELECT payload_json FROM resolver_cache"
        ).fetchall()

        self.assertEqual(identity.tmdb_id, 1734)
        self.assertTrue(rows)
        serialized = "\n".join(str(row["payload_json"]) for row in rows)
        self.assertNotIn(raw_source_title, serialized)
        self.assertNotIn("source_title\"", serialized)
        self.assertIn("source-title:", serialized)

    def test_conflicting_source_titles_are_rejected_instead_of_selecting_one(self):
        mummy = movie_payload(
            1734, "El regreso de la momia", "The Mummy Returns", 2001, "en"
        )
        alien = movie_payload(
            348, "Alien, el octavo pasajero", "Alien", 1979, "en"
        )

        def search(params):
            query = str(params.get("query") or "").lower()
            if "momia" in query:
                return {"results": [mummy]}
            if "alien" in query:
                return {"results": [alien]}
            return {"results": []}
        ...
        input_root = self.input_file("release-irreconocible.mkv")
        infohash = "a" * 40
        job = {
            "name": "release-irreconocible.mkv",
            "category": "movies",
            "infohash": infohash,
            "source_meta_json": json.dumps(
                {
        ...
        with self.assertRaises(ResolverAmbiguous) as raised:
            resolver.resolve(job, input_root)

        self.assertEqual(
            raised.exception.details["reason_code"], "source_context_conflict"
        )
        self.assertEqual(
            {
                attempt["tmdb_id"]
                for attempt in raised.exception.details["source_fallback_attempts"]
            },
            {1734, 348},
        )
